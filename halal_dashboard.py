...
def generate_dynamic_portfolios(stock_data, monthly_sip, risk_profile="Balanced", strategy="Growth (Momentum)"):
    ...
    # Attempt to load the massive 2700+ metrics file generated by GitHub Actions
    universe_df = None
    try:
        import json
        import pandas as pd
    ...
    def waterfall_allocate(pool_df, price_col="Live Price (₹)", sym_col="Symbol"):
        ...
        for _, row in pool_df.iterrows():
            ...
            holdings_dict[sym] = {
                "qty": 0, 
                "price": row[price_col], 
                "sector": row.get("sector", "Unknown") if "sector" in row else HALAL_STOCKS.get(sym+".NS", {}).get("sector", "Unknown"),
                "color": "#00F0FF" # Default color
            }
    ...
    if universe_df is not None and not universe_df.empty:
        # Filter based on Risk Tolerance (Beta)
        ...
        if "Value" in strategy:
            ...
            # Growth: Assume high market cap and high beta (momentum proxy since we lack historical on 2700)
            filtered_df = filtered_df.sort_values(['beta', 'market_cap'], ascending=[False, False])
            
        # Select top 12 assets that fit the SIP
        pool = filtered_df.head(20).copy()
        # Ensure we have price col mapping
        pool["Symbol"] = pool["symbol"]
        pool["Live Price (₹)"] = pool["price"]
        
        # Build 3 time horizons using subsets
        p1 = pool.head(10)
        ...
        # FALLBACK: Use top 57 hardcoded logic if JSON is missing
